...
static bool	isMiddle(const std::string &middle) {
	if (middle.empty()) {
		return (false);
	}
	if (middle[0] == ':') {
	...
	for (size_t i = 0; i < middle.size(); i++) {
		if (!isNotSpaceCrLfColonChar(middle[i]) && middle[i] != ':') {
			return (false);
		}
	}
	return (true);
...
// nospcrlfcl	=	%x01-09 / %x0B-0C / %x0E-1F / %x21-39 / %x3B-FF
// 					; any octet except NUL, CR, LF, " " and ":"
static bool	isTrailing(const std::string &trailing) {
	if (trailing.empty()) {
		return (false);
		// return (true);
	}
	...
	for (size_t i = 0; i < trailing.size(); i++) {
		if (isNotSpaceCrLfColonChar(trailing[i])) {
			continue;
		}
		if (trailing[i] == ':' && trailing[i] == ' ') {
			continue;
		}
		return (false);
	}
...
// int	Parser::parse(std::string message, const Info& info) {
	...
	try {
		...
		if (this->parsed_.getCommand() == "PING") {
			...
			} else if (!isParamChannel(tokens[1].getValue())) {
			...
			if (tokens.size() == 4) {
				...
				if (tokens[2].getValue()[1] == 'i') {
					if (!isParamNickName(tokens[3].getValue())) {
						// 432 ERR_ERRONEUSNICKNAME "<nick> :Erroneous nickname"
						return (-1);
					}
				// Key		<modeparam> == <key>
				} else if (tokens[2].getValue()[1] == 'k') {
					if (!isParamKey(tokens[3].getValue())) {
						return (-1);
					}
				// Limit	<modeparam> == digit(< 5)
					...
					if (!isFuncString(tokens[3].getValue(), isNotDigit)) {
						return (-1);
					}
				...
				// Topic	<modeparam> == <topic>
					...
					if (!isParamTopic(tokens[3].getValue())) {
						return (-1);
					}
				}
			}
		...
		// PRIVMSG <msgtarget> <text>
			...
			// if (!isTrailing(tokens[2].getValue())) {
			// 	return (-1);
			// }
